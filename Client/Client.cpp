...
int main()
{
    ...
    strcpy_s(buff_send, data.toStyledString().c_str());
    ...
    while(1)
    {
       ...
       // MessageBox(NULL, data.toStyledString().c_str(), "JSON_DATA", MB_ICONWARNING | MB_OK);
        ...
        case E_ATTACK:
            ...
            memset(E_Buff, 0, 200);
            strcpy_s(E_Buff, data.toStyledString().c_str());
        ...
        case SERVER_MSG:
            ...
            MessageBoxA(NULL, data["message"].asString().c_str(), data["title"].asString().c_str(), MB_ICONWARNING | MB_OK);
            break;
        }
        data.clear();
    }
